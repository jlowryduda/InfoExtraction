...
import edu.umass.cs.mallet.base.types.ConstantVector;
...
public class KMeans extends Clusterer {
	       ...
	       deltaPoints > instances.size()*POINTS_TOLERANCE)
	{
		...
		for (int c=0 ; c<numClusters ; c++ ) {
		    ...
		    instDist = metric.distance( (ConstantVector)clusterMeans.get(c), 
						(ConstantVector)instances.getInstance(n).getData(instancePipe) );
		...
		if (((InstanceList)instanceClusters.get(c)).size()>0) {
		    clusterMean = ConstantVector.mean( (InstanceList)instanceClusters.get(c) );
		    
		    deltaMeans += metric.distance( (ConstantVector)clusterMeans.get(c), clusterMean );
