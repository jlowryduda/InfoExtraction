 ...
 */
   ...
   */
  public static CRF3 train(InstanceList training, InstanceList testing,
                           TransducerEvaluator eval, int[] orders,
                           String defaultLabel,
                           String forbidden, String allowed,
                           boolean connected, int iterations, double var)
  {
    Pattern forbiddenPat = Pattern.compile(forbidden);
    Pattern allowedPat = Pattern.compile(allowed);
    CRF3 crf = new CRF3(training.getPipe(), null);
    String startName =
  ...
  /**
   * Apply a transducer to an inpput sequence to produce the highest-scoring
   * output sequence.
   *
   * @param model the <code>Transducer</code>
   * @param input the input sequence
   * @return the best scoring output sequence
   */
  public static Sequence apply(Transducer model, Sequence input)
  {
    return model.viterbiPath(input).output();
  }
    ...
    else
    {
      ...
      else
      {
        for (int i = 0; i < testData.size(); i++)
        {
          ...
          Sequence output = apply(crf, input);
          if (output.size() != input.size())
            System.out.println("Failed to decode input sequence " + i);
          else
          {
